...
const EmailTemplates = () => {
    ...
    const [selectedFold, setSelectedFold] = useState('all')
    ...
    const onSelectChange = async (value: string) => {
        setSelectedFold(value)
        setSearchVal('')
        // 这里调整跳转逻辑，当选择一个folder时，将搜索框清空，并且不携带keyword查询
        message.loading({content: 'loading', duration: 10, key: 'listLoading'})
        const query = {folder: value, keyword: ''}
        const res = await getTemplateList(query)
        message.destroy('listLoading')
        if (res.code === SUCCESS_CODE) {
            const data = res.data
    ...
    const onSearch = async () => {
        // 这里逻辑为：当在xxFolder页面时，就携带对应的folder跟查询条件查询，
        // 当查询条件为空时，则查询出所有xxFolder标签下的template返回
        let res;
        message.loading({content: 'loading', duration: 10, key: 'listLoading'})
        const query = {folder: selectedFold, keyword: searchVal}
        res = await getTemplateList(query)
        message.destroy('listLoading')
        if (res.code === SUCCESS_CODE) {
            const data = res.data
            const newTemplateList = []
            ...
            <div className={content}>
                ...
                </div>
                {
                    templateList.map((item, index) => <div key={index} className={templateWrapper}>
                        <div className={templateTitle}>{item.folder === 'dealsAndOffers' ? 'deals & offers' : item.folder}</div>
                        <div className={itemWrapper}>
                            {item.list.map((i, idx) => <div key={idx}
                                                            onClick={() => router.push(`/editTemplate?id=${i.id}`)}
                                                            className={itemBox}>
                                <div className={cover}>
                                    {
                                        item.folder === 'public' ? (
                                            <img src="/img/template_img_public.png" alt="public template img"/>
                                        ) : i.folder === 'welcome' ? (
                                            <img src="/img/template_img_welcome.png" alt="welcome template img"/>
                                        ) : i.folder === 'seasons' ? (
                                            <img src="/img/template_img_seasons.png" alt="seasons template img"/>
                                        ) : i.folder === 'dealsAndOffers' ? (
                                            <img src="/img/template_img_dealsAndOffers.png"
                                                 alt="dealsAndOffers template img"/>
                                        ) : (
                                            <p>我是封面</p>
                                        )
                                    }
