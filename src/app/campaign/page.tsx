...
const Campaign = () => {
    ...
    const [campaignList, setCampaignList] = useState<(Campaign.CampaignNew & {
        status: number,
        updateTime: string
    })[]>([])
        ...
        if (res.code === SUCCESS_CODE) {
            ...
            if (setCampaignList.length <= 1 && currentPage > 1) {
                ...
                if (newListRes.code === SUCCESS_CODE) {
                    setCampaignList(newListRes.data?.records.map((item: { id: number }) => ({
                        ...item,
                        key: item.id
                    })) || []);
                }
            ...
            <div className={content}>
                <div className={viewWrapper}>
                    ...
                    <div onClick={() => setStatus('')}
                         className={classNames(statusItem, {[active]: status === ''})}>All
                    </div>
                    <div onClick={() => setStatus('1')}
                         className={classNames(statusItem, {[active]: status === '1'})}>Ongoing
                    </div>
                    <div onClick={() => setStatus('0')}
                         className={classNames(statusItem, {[active]: status === '0'})}>Draft
                    </div>
                    <div onClick={() => setStatus('2')}
                         className={classNames(statusItem, {[active]: status === '2'})}>Completed
                    </div>
                </div>
                                        ...
                                        <div className={listItem}>
                                            <div className={listLeft}>
                                                ...
                                                <div className={itemDescBox}>
                                                    <Link href={`/campaignEditor?id=${record.id}`}
                                                          className={record.status === 0 ? itemTitle : noItemTitle}>{record.campaignName}</Link>
                                            ...
                                            <div className={listRight}>
                                                <Link href={`/campaignEditor?id=${record.id}`}
                                                      className={record.status === 0 ? editBtn : noEditBtn}>Edit</Link>
                                                <div onClick={() => onDelete(record.id)} className={editBtn}>Delete
                                                </div>
                                            </div>
                                        </div>
                                    ),
