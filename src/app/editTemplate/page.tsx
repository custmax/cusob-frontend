...
import {Form, Input, Radio, RadioChangeEvent, message, Select, Button, Modal} from 'antd';
import ImgWrapper from '@/component/ImgWrapper';
import React, {useEffect, useRef, useState} from 'react';
import dynamic from 'next/dynamic'
import {getTemplate, removeCustomizedTemplate, saveTemplate, updateTemplate} from '@/server/template';
...
import {removeTemplate} from "@/server/sendcloud/template";
...
const EditTemplate = () => {
    ...
    const onDeleteButtonClick = async () => {
        ...
        Modal.confirm({
            ...
            centered: true,
            onOk: async () => {
                message.loading({content: 'loading', duration: 10, key: 'removeTemplateLoading'})
                const res = await removeCustomizedTemplate(templateId)
                message.destroy('removeTemplateLoading')
                if (res.code === SUCCESS_CODE) {
                    message.success(res.message, () => {
                        router.back()
                    })
            ...
            <div className={main}>
                <div className={title}>
                    ...
                    <div className={operateBox}>
                        {
                            templateId ? (
                                    originTemplate.isCustomized === 1 ? (<Button className={templateDeleteButton} type="primary" danger onClick={onDeleteButtonClick}>
                                        Delete
                                    </Button>) : (<Button className={templateDeleteButton} type="primary" danger disabled>
                                        Delete
                                    </Button>)
                                ) :
                                <p></p>
