...
const CampaignEditor = () => {
    ...
    let [senderId, setSenderId] = useState<any>()
    ...
    const initCampaign = useCallback(async (groupList: { value: number, label: string }[]) => {
        if (campaignId) {
            ...
            if (res.code === SUCCESS_CODE && res.data) {
                ...
                setDesignContent(res.data.designContent)
    ...
    const initSender = useCallback(async () => {
        ...
        if (res.code === SUCCESS_CODE && res.data) {
            ...
            if (senderListRef.current && senderListRef.current.length > 0) {
                //console.log("senderListRef:",senderListRef.current[senderListRef.current.length - 1].value)
                senderId = senderListRef.current[senderListRef.current.length - 1].value
                setSenderId(senderListRef.current[senderListRef.current.length - 1].value);
                //console.log("senderId111:",senderId)
                onFromOk();
                //console.log("senderId222:",senderId)
            }
        }
    }, [])
...
// 监听senderId的变化
    ...
    const onContentOk = () => {
        ...
        //console.log('Saved Design JSON:', designContent);
        //console.log('Saved Content:', richContent);
        setProcess(newProcess)
    ...
    return <div className={campaignEditorContainer}>
        ...
        <Modall
            ...
            onDesignChange={(design) => setDesignContent(design)}
            designContent={designContent ? JSON.stringify(designContent) : null}
        />
        <Modal title="Send Failed." open={isAlertModalOpen} onCancel={handleAlertModalCancel} cancelText="Confirm"
               okButtonProps={{style: {display: "none"}}} closable={false}>
            <Alert
                message="The number of emails sent from this address has exceeded the limit. "
                description={`You can send ${restValue} more emails today using this email.`}
                type="error"
                showIcon
                closable={false}
