...
const ContactList = () => {
    ...
    const [isAddGroupProcessing, setIsAddGroupProcessing] = useState(false)
    const [isEditGroupProcessing, setIsEditGroupProcessing] = useState(false)
    const [isDeleteGroupProcession, setIsDeleteGroupProcession] = useState(false)
...
// 如果请求成功，更新联系人列表和当前页码。
    ...
    const onGroupOk = async () => {
        if (isAddGroupProcessing) return
        setIsAddGroupProcessing(true)
        if (groupName) {
    ...
    const onEditGroupOk = async () => {
        if (isEditGroupProcessing) return
        setIsEditGroupProcessing(true)
        if (editGroupName && activeGroupId !== -1) {
            const res = await updateGroup(editGroupName, activeGroupId)
            if (res.code === SUCCESS_CODE) {
                // 由于调用initList时会弹出加载中的标签，所以需要休眠等待修改成功的信息结束后才initList
        ...
        setIsEditGroupProcessing(false)
    }

    const onEditGroupDelete = async () => {
        if (isDeleteGroupProcession)
            return
        setIsDeleteGroupProcession(true)
        if (activeGroupId !== -1) {
        ...
        setIsDeleteGroupProcession(false)
    }
    ...
    return <div className={contactListContainer}>
        ...
        <Modal
            title="Add Group"
            open={showGroup}
            onOk={!isAddGroupProcessing ? onGroupOk : undefined}
        ...
        <Modal
            ...
            footer={() => <div>
                ...
                <Button onClick={!isEditGroupProcessing ? onEditGroupOk : undefined} type='primary'>Ok</Button>
            </div>}
        >
            <div className={groupContent}>
                <div className={inputItem}>
